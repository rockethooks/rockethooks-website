---
title: "WooCommerce Integration"
description: "Monitor WooCommerce orders, products, and customers with real-time change detection. Transform WooCommerce REST API endpoints into event streams."
platform: "WooCommerce"
logo: "/images/integrations/woocommerce.svg"
category: "E-commerce"
featured: false
documentationUrl: "https://woocommerce.github.io/woocommerce-rest-api-docs/"
---

## How RocketHooks Works with WooCommerce

RocketHooks connects to the WooCommerce REST API to monitor your store for order activity, product changes, and customer updates. WooCommerce exposes a comprehensive REST API that RocketHooks polls at your configured interval, detecting changes through JSONPath expressions or full response comparison.

Compatible with **WooCommerce 3.5+** with REST API enabled (enabled by default).

## Authentication Setup

WooCommerce uses API key authentication. Generate keys in WooCommerce > Settings > Advanced > REST API:

```json
{
  "auth": {
    "type": "basic",
    "username": "YOUR_CONSUMER_KEY",
    "password": "YOUR_CONSUMER_SECRET"
  }
}
```

For HTTPS sites (recommended), use the keys directly. For HTTP development sites, WooCommerce uses OAuth 1.0a -- RocketHooks handles both.

## Endpoint Examples

### Monitor New Orders

Track incoming orders as they arrive:

```json
{
  "url": "https://your-store.com/wp-json/wc/v3/orders?per_page=20&orderby=date&order=desc",
  "method": "GET",
  "pollInterval": 60,
  "changeDetection": {
    "strategy": "jsonpath",
    "expression": "$[*].id"
  }
}
```

### Monitor Order Status Changes

Detect when orders transition between statuses (pending, processing, completed):

```json
{
  "url": "https://your-store.com/wp-json/wc/v3/orders?per_page=50&orderby=modified&order=desc",
  "method": "GET",
  "pollInterval": 120,
  "changeDetection": {
    "strategy": "jsonpath",
    "expression": "$[*].status"
  }
}
```

### Monitor Product Stock

Watch for inventory changes across your catalog:

```json
{
  "url": "https://your-store.com/wp-json/wc/v3/products?per_page=100&orderby=modified&order=desc",
  "method": "GET",
  "pollInterval": 300,
  "changeDetection": {
    "strategy": "jsonpath",
    "expression": "$[*].stock_quantity"
  }
}
```

## Common Patterns

### Multi-Channel Order Processing

Monitor WooCommerce orders alongside Shopify or other storefronts. RocketHooks normalizes the change events so your fulfillment pipeline receives a consistent webhook payload regardless of the source.

### Abandoned Cart Recovery

Track order status changes. When orders stay in "pending" status beyond a configured time, trigger email campaigns or Slack alerts to follow up with customers.

### Inventory Sync

Keep WooCommerce inventory synchronized with warehouse management systems. When stock levels change, RocketHooks notifies your WMS to update its records.

## Delivery Channels

- **Webhooks** -- Connect to fulfillment or accounting systems
- **Slack** -- Real-time team notifications for new orders
- **Email** -- Order summaries or stock alerts for managers
- **SMS** -- Critical alerts for high-value orders

## Next Steps

1. Generate WooCommerce REST API keys (read permissions minimum)
2. Add the consumer key and secret to RocketHooks
3. Configure your first order or product monitor
4. Set up notification channels for your workflow
